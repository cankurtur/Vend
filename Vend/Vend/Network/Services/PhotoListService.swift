...
protocol PhotoListServiceable {
    func getPhotos(page: Int, limit: Int) async ->  Result<[PhotoResponse], APIClientError>
}
...
actor PhotoListService: PhotoListServiceable {
    ...
    func getPhotos(page: Int, limit: Int) async ->  Result<[PhotoResponse], APIClientError> {
        do {
            let response = try await networkManager.request(endpoint: .getPhotos(page: page, limit: limit), responseType: [PhotoResponse].self)
